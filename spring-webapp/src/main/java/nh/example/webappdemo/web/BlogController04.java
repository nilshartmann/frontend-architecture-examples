...
@Controller
@RequestMapping("/steps/04")
public class BlogController04 {

    private static final Logger log = LoggerFactory.getLogger(BlogController04.class);

    private final BlogPostRepository blogPostRepository;
    private final CommentService commentService;

    public BlogController04(BlogPostRepository blogPostRepository, CommentService commentService) {
    ...
    @GetMapping("")
    public String index() {
        return "redirect:/steps/04/post/1";
    }
    ...
    private ModelAndView populateModel(int postId) {
        var modelAndView = new ModelAndView("/steps/04/blogpost");
        ...
        if (postId > 1) {
            model.addAttribute("prevUrl", "/steps/04/post/" + (postId - 1));
        }
        if (postId < blogPostRepository.getPostCount()) {
            model.addAttribute("nextUrl", "/steps/04/post/" + (postId + 1));
        }
