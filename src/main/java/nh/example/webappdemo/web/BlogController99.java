...
@Controller
@RequestMapping("/steps/99")
public class BlogController99 {

    private static final Logger log = LoggerFactory.getLogger(BlogController99.class);

    private final BlogPostRepository blogPostRepository;
    private final CommentService commentService;

    public BlogController99(BlogPostRepository blogPostRepository, CommentService commentService) {
    ...
    @GetMapping("")
    public String index() {
        return "redirect:/steps/99/post/1";
    }
    ...
    private ModelAndView populateModel(int postId) {
        var modelAndView = new ModelAndView("/steps/99/blogpost");
        ...
        if (postId > 1) {
            model.addAttribute("prevUrl", "/steps/99/post/" + (postId - 1));
        }
        if (postId < blogPostRepository.getPostCount()) {
            model.addAttribute("nextUrl", "/steps/99/post/" + (postId + 1));
        }
